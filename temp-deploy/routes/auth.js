...
router.post('/register', validateRegistration, async (req, res) => {
  try {
    ...
    const result = await db.query(
      ...
      [userId, email, passwordHash, name, role, isAutoApproved] // Auto-approve students and admins, others need approval
    );
    ...
    res.status(201).json({
      ...
      token,
      requiresApproval: (role === 'instructor' || role === 'staff')
    });
...
router.post('/login', validateLogin, async (req, res) => {
  try {
    ...
    // Check if user is approved (for instructors and staff only - admins and students are auto-approved)
    if ((user.role === 'instructor' || user.role === 'staff') && !user.is_approved) {
      console.log(`🚫 Login blocked - ${user.role} user ${user.email} not approved`);
      return res.status(403).json({ 
        error: 'Account pending approval',
        message: `Your ${user.role} account is pending approval from an administrator`
      });
    }
