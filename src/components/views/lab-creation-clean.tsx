                                        ...
                                        {labData.icon && (
                                            <div className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                                <div className="flex-shrink-0">
                                                    <img
                                                        src={labData.icon.startsWith('/uploads/labs/')
                                                            ? (() => {
                                                                // Extract lab name and filename from path like "/uploads/labs/labName/filename.png"
                                                                const pathParts = labData.icon.split('/');
                                                                if (pathParts.length >= 4) {
                                                                    const labName = pathParts[3]; // "labName"
                                                                    const filename = pathParts[4]; // "filename.png"
                                                                    return `http://localhost:3001/api/files/labs/${labName}/${filename}`;
                                                                }
                                                                return labData.icon; // fallback
                                                            })()
                                                            : labData.icon
                                                        }
